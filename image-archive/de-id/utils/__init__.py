...
def dicom_to_dict(dicom, log=None):
  ...
  for key, value in dicom_metadata.items():
    ...
    log.debug('%s: %s' % (key, value)) if log
  ...
  if 'TransferSyntaxUID' not in dicom.file_meta:
    ...
    log.warning('Assumed TransferSyntaxUID') if log
  ...
  try:
    ...
    log.warning('Didn\'t understand value: %s = \'%s\'' % ('PatientBirthDate', dicom_metadata['PatientBirthDate'])) if log
    log.warning('Problem image was: %s\n' % filepath) if log
  ...
  try:
    ...
    log.warning('Didn\'t understand value: %s = \'%s\'' % ('AcquisitionDate', dicom_metadata['AcquisitionDate'])) if log
    log.warning('Problem image was: %s\n' % filepath) if log
  ...
  try:
    ...
    log.warning('Falling back for PatientAge') if log
  ...
  try:
    ...
    log.warning('Didn\'t understand value: %s = \'%s\'' % ('PatientAge', dicom_metadata['PatientAge'])) if log
    log.warning('Problem image was: %s\n' % filepath) if log
  if ENVIRON=='local':
