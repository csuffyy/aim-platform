...
nohup sh -c 'while :; do ssh -Nq -R 0.0.0.0:8000:localhost:8000 ubuntu@172.20.4.83; done > ~/ssh8000.log' &
...
nohup sh -c 'while :; do ssh -Nq -L 0.0.0.0:9200:localhost:9200 ubuntu@172.20.4.83; done > ~/ssh9200.log' &
