...
ssh -fNq -R 0.0.0.0:8000:localhost:8000 ubuntu@172.20.8.15
nohup sh -c 'while :; do ssh -Nq -R 0.0.0.0:8000:localhost:8000 ubuntu@172.20.8.15; done > ~/ssh8000.log' &

# AIM tunnel elasticsearch running on OpenStack VM (port 9200) to HPF data server (port 9200)
# Run on hpf23.ccm.sickkids.ca
ssh -fNq -L 0.0.0.0:9200:localhost:9200 ubuntu@172.20.8.15
nohup sh -c 'while :; do ssh -Nq -L 0.0.0.0:9200:localhost:9200 ubuntu@172.20.8.15; done > ~/ssh9200.log' &
